...
| `notification_log` | No age purge. Rows cascade on node or Auth-user deletion. Delivered messages at external providers/recipients remain outside this table. | Lifecycle cascade only | Approve and implement an operational/security retention period. Minimise target, subject and error text. |
