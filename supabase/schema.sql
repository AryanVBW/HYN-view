...
-- ===========================================================================
-- let a maintainer dismiss a false alert (migrations/20260922070000_maintainer_resolve_alert.sql)
-- ===========================================================================
-- Every other write RPC in this file uses _hyn_require_staff() or
-- _hyn_require_admin(), which gate on hyn_is_admin()/hyn_is_super_admin() -- not
-- on fleet visibility. Reusing either here would hand a Maintainer role
-- assignment, suspension and node deletion just to let them clear a warning, so
-- this gets its own guard.
...
-- was a one-off backup job". Idempotent, and scoped to the alert's own node
-- through hyn_can_view_node() so this cannot touch a row outside what the
-- caller's fleet visibility already covers.
