...
# Access activity is useful health context, but usernames and source addresses
# are identity-bearing data. Reports therefore show aggregate counts by default
# and include the forensic detail only after an explicit local opt-in.
report_access_text() {
  local hours=${1:-24}
  ...
  if ((FAILED_LOGINS > 0)); then
    printf '  rejected logins  %s in %sh%s\n' "$FAILED_LOGINS" "$hours" \
      "$( ((access_detail)) && [[ -n $FAILED_LOGIN_TOP ]] && printf '   worst: %s' "$FAILED_LOGIN_TOP" )"
...
report_html() {
...

  {
    ...
    if ((FAILED_LOGINS > 0)); then
      ...
      local rejected="$FAILED_LOGINS"
      ((access_detail)) && [[ -n $FAILED_LOGIN_TOP ]] && rejected+=" · worst $FAILED_LOGIN_TOP"
      e_kv "Rejected logins (${hours}h)" "$rejected" "$fcol"
    fi
    e_kv_close
    if ((access_detail && ${#SESS_USER[@]} > 0)); then
