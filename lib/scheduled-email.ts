...
// Evaluate recurring email after each accepted full telemetry upload. This
// honors a user's local schedule within their configured telemetry interval on
// Vercel plans whose platform cron may run only once per day. The daily cron
...
export async function dispatchScheduledEmails(nodeId?: string): Promise<ScheduledEmailResult> {
  ...
  if (preferencesResult.error || templatesResult.error) {
    throw preferencesResult.error ?? templatesResult.error;
  }
  ...
  const result: ScheduledEmailResult = {
    ...
    failed: 0,
    skipped: 0,
    checked: preferencesResult.data?.length ?? 0,
  };

  async function deliver(args: {
    preference: PreferenceRow;
    kind: "alert" | "report" | "system";
    idempotencyKey: string;
    subject: string;
    severity: "info" | "warn" | "crit";
    content: string;
  }) {
    const claim = await supabase.from("cloud_email_dispatches").insert({
      idempotency_key: args.idempotencyKey,
      node_id: args.preference.node_id,
      kind: args.kind,
    });
    if (claim.error) {
    ...
    await supabase.from("notification_log").insert({
      ...
      owner: node.owner,
      kind: "resend-cloud",
      target: args.preference.recipient,
      severity: args.severity,
      subject: args.subject,
      status: ok ? "sent" : "failed",
      error: ok ? null : delivery.error,
      category: args.kind === "alert" ? "alert" : "report",
    });
    if (!ok) {
      await supabase
    ...
    await supabase
      .from("cloud_email_dispatches")
      .update({ provider_id: delivery.providerId })
      .eq("idempotency_key", args.idempotencyKey);
    result.sent += 1;
    return true;
  }
  ...
  for (const rawPreference of preferencesResult.data ?? []) {
    ...
    if (preference.incident_enabled) {
      const alertResult = await supabase
        .from("alert_events")
        .select("id,rule,ts,severity,message,resolved")
        .eq("node_id", node.id)
        .gt("id", preference.last_alert_id)
        ...
        if (incoming.length) {
          ...
          const previousResult = preference.last_alert_id > 0
            ? await supabase
                ...
                .select("id,rule,ts,severity,message,resolved")
                .eq("node_id", node.id)
                .lte("id", preference.last_alert_id)
          ...
          if (previousResult.error) {
            ...
            if (events.length === 0) {
              ...
              const ok = await deliver({
                preference,
                kind: "alert",
                idempotencyKey: `alert:${node.id}:${lastId}`,
                subject: `[HYN ${severity.toUpperCase()}] ${node.name}: ${events.length} incident update${events.length === 1 ? "" : "s"}`,
                severity,
                content: buildIncidentContent(events),
              });
              if (ok) {
                await supabase.from("email_preferences").update({ last_alert_id: lastId }).eq("node_id", node.id);
              }
    ...
    const dailyDue = preference.daily_enabled && scheduleIsDue(
      ...
      preference.last_daily_local_date,
    );
    const systemDue = preference.system_enabled && scheduleIsDue(
      now,
      preference.timezone,
      preference.system_at.slice(0, 5),
      preference.last_system_local_date,
    );
    if (!dailyDue && !systemDue) continue;

    const metricResult = await supabase
      .from("metrics")
      .select("cpu_pct,cpu_temp_c,mem_pct,net_rx_bps,net_tx_bps,latency_ms,uptime_s,payload,ts")
      .eq("node_id", node.id)
      .gte("ts", since)
    ...
    if (dailyDue) {
      const ok = await deliver({
        preference,
        kind: "report",
        idempotencyKey: `report:${node.id}:${local.date}`,
        subject: `Daily HYN health · ${node.name} · ${local.date}`,
        severity: "info",
        content: buildDailyDigestContent({
          nodeName: node.name,
          sampleCount: metrics.length,
          cpuAverage: average(metrics.map((row) => row.cpu_pct)),
          cpuPeak: peak(metrics.map((row) => row.cpu_pct)),
          memoryAverage: average(metrics.map((row) => row.mem_pct)),
          temperaturePeak: peak(metrics.map((row) => row.cpu_temp_c)),
          downloadAverageBps: average(metrics.map((row) => row.net_rx_bps)),
          uploadAverageBps: average(metrics.map((row) => row.net_tx_bps)),
          latencyAverageMs: average(metrics.map((row) => row.latency_ms)),
          uptimeSeconds: latest?.uptime_s ?? null,
        }),
      });
      if (ok) {
        await supabase
          .from("email_preferences")
          .update({ last_daily_local_date: local.date })
          .eq("node_id", node.id);
      }
    }
    ...
    if (systemDue) {
      const ok = await deliver({
        preference,
        kind: "system",
        idempotencyKey: `system:${node.id}:${local.date}`,
        subject: `System information · ${node.name} · ${local.date}`,
        severity: "info",
      ...
      if (ok) {
        await supabase
          .from("email_preferences")
          .update({ last_system_local_date: local.date })
          .eq("node_id", node.id);
      }
    }
  }
