...
// Ported from React Bits' free, open-source MagicRings (reactbits.dev/backgrounds/magic-rings,
// JS+CSS variant) to TypeScript. The shader, uniform wiring, animation loop, and the
// visibility/page-visibility gating (IntersectionObserver + document.visibilitychange, so the
// GPU stops working the moment the effect scrolls off-screen or the tab is backgrounded) are
// unchanged from upstream -- `three` is already a dependency here (see components/gl), so this
// needed no new package.
//
// Not mounted sitewide the way SiteCursorGrid was: this is a per-page background for the
// authenticated app surfaces specifically (dashboard, account, admin), not the public marketing
// pages, so it is rendered where those pages already mount ParticleField rather than once in the
// root layout.
...
export default function MagicRings({
  ...
  // Same shape as CursorGrid's own propsRef sync: an unconditional effect
  // rather than a ref write during render, which is what this codebase's
  // stricter react-hooks/refs rule requires (see cursor-grid.tsx for the
  // fuller explanation -- refs are meant to be written outside of render).
  useEffect(() => {
    propsRef.current = {
      color,
      colorTwo,
  ...
  useEffect(() => {
    ...
    const resize = () => {
      // Falls back to the viewport size when the mount div's own clientWidth
      // /clientHeight come back 0 -- which is exactly what happened here:
      // upstream trusts the mount element's box unconditionally, but a div
      // sized purely by CSS percentage height (h-full) nested inside a
      // `position: fixed` ancestor can still read 0 the instant this effect
      // first runs, before the browser has committed that layout. The
      // canvas then gets sized to that 0 (or whatever tiny value) and every
      // frame paints into that small box at the top-left of the page instead
      // of the full viewport -- see particle-field.tsx's own resize(), which
      // already carries this exact fallback for the same reason.
