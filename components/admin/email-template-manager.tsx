        ...
        <nav className="border-b border-border bg-muted/20 p-3 lg:border-r lg:border-b-0" aria-label="Email templates">
          {templates.map((template) => (
            <button
              ...
              className={`mb-2 w-full border p-3 text-left transition-colors ${
        ...
        <div className="min-w-0 p-4 md:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex border border-border">
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 font-mono text-xs ${view === "code" ? "bg-primary/10 text-primary" : "text-muted-foreground"}`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 font-mono text-xs ${view === "preview" ? "bg-primary/10 text-primary" : "text-muted-foreground"}`}
              >
          ...
          <div className="mt-5">
            {view === "code" ? (
              <textarea
                ...
                spellCheck={false}
                className="min-h-[440px] w-full resize-y border border-input bg-background p-4 font-mono text-xs leading-6 text-foreground outline-none transition-colors focus:border-primary"
              />
            ) : (
              <iframe
                ...
                className="h-[440px] w-full border border-border bg-white"
              />
            )}
          </div>
          ...
          <div className="mt-4 flex flex-col gap-4 border-t border-border pt-4 md:flex-row md:items-end md:justify-between">
            ...
            <button
              ...
              disabled={pending}
              onClick={save}
              className="flex min-w-40 items-center justify-center gap-2 border border-primary bg-primary/10 px-4 py-2.5 font-mono text-xs uppercase text-primary transition-colors hover:bg-primary/20 disabled:cursor-wait disabled:opacity-60"
            >
          ...
          {message ? (
            <p role="status" className={`mt-4 border p-3 font-mono text-xs leading-5 ${message.tone === "ok" ? "border-primary/40 bg-primary/5 text-primary" : "border-destructive/40 bg-destructive/5 text-destructive"}`}>
              {message.text}
            </p>
          ) : null}
        </div>
